...
![A yellow sticky note card with the title "Accuracy and precision are not the same thing." There are four bow and arrow targets and under each are two titles that can be crossed out if they do not apply: Accuracy and Precision. There are groupings of red dots on each of the targets to demonstrate the difference. The first target has some dots sparsely grouped to the bottom left and another dot on the top right. Accuracy and precision are both crossed out. The second has less sparse grouping around the center of the target with accurate in plain text and precise crossed out. There is an arrow above with it with text "Accuracy, true to intention". The third target has tightly grouped dots to the upper left of the target and accurate is crossed out, precise is left in plain text. There is a line and arrow pointing to this target with the text "precision, true to itself", above it. The final target has a tightly grouped set of dots around the center of the target with both accurate and precise in plain text.][SKETCH]
